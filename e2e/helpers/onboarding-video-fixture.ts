...
export async function mockOnboardingVideoApi(page: Page, options: { complete?: boolean; completedSteps?: number } = {}) {
  ...
  await page.route("http://localhost:3333/**", async (route) => {
    ...
    if (url.pathname === "/auth/login" && method === "POST") return json({ accessToken: "video-demo-access-token", expiresIn: "1d", user: { id: "video-user", name: "Video Teste", email: "video@example.test", role: "admin", roles: ["admin"] }, tenant: { id: "video-tenant", name: "Tenant Video", timezone: "America/Sao_Paulo", slug: "tenant-video" } });
    ...
    if (url.pathname === "/professionals" && method === "POST") { professionalCreated = true; return json({ professional: demoProfessional }, 201); }
    ...
    if (url.pathname === "/tenants/me/customer-app-settings") return json({ customerAppUrl: `${publicOrigin}/c/tenant-video`, whatsappMessageTemplate: "Acesse sua agenda: https://example.test/c/tenant-video", whatsappBusinessHint: "Mensagem pronta para compartilhar." });
    ...
    if (url.pathname === "/public/professionals/maria-teste/available-dates") return json({ tenantTimezone: "America/Sao_Paulo", availableDates: ["2026-08-31"] });
    if (url.pathname === "/public/professionals/maria-teste/slots") return json({ tenantTimezone: "America/Sao_Paulo", slots: [{ start: "2026-08-31T13:00:00.000Z", end: "2026-08-31T14:00:00.000Z" }] });
    if (url.pathname === "/public/bookings" && method === "POST") return json({ id: "booking-video", professionalId: "professional-video", serviceId: "service-video", start: "2026-08-31T13:00:00.000Z", end: "2026-08-31T14:00:00.000Z", status: "CONFIRMED", depositRequired: false, depositStatus: "NOT_REQUIRED", customerName: "Cliente de Teste", customerPhone: "+5511999999999", professionalName: "Maria Demo", serviceName: "Consulta de demonstracao", customerAppBootstrapToken: "bootstrap-video", cancelToken: "cancel-video" });
    return json({});
  });
}
