...
test("POC didatico: cadastrar o primeiro profissional", async ({ page }) => {
  ...
  await demoType(page, page.getByLabel("Email"), "onboarding-professional-demo@example.test", 45);
  await demoType(page, page.getByLabel("Senha"), "Onboarding123!", 55);
  ...
  const result = page.getByText("Maria Demo", { exact: true });
