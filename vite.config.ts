...
function resolveHttpsConfig(mode: string) {
  ...
  if (!keyPath || !certPath) {
    throw new Error(
      "VITE_DEV_HTTPS=true exige VITE_DEV_HTTPS_KEY_PATH e VITE_DEV_HTTPS_CERT_PATH apontando para os arquivos do certificado local.",
    );
  }
  ...
  if (!fs.existsSync(resolvedKeyPath) || !fs.existsSync(resolvedCertPath)) {
    throw new Error(
      `Certificado HTTPS local nao encontrado. key=${resolvedKeyPath} cert=${resolvedCertPath}`,
    );
  }
