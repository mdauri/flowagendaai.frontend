...
export function ApiTokensPanel({ tenantId, onDirtyChange }: ApiTokensPanelProps) {
  const { toast } = useToast();
  const listQuery = useApiTokensQuery(tenantId);
  ...
  const isDirty = useMemo(() => {
    return (
      form.name !== emptyFormState.name ||
      form.prefix !== emptyFormState.prefix ||
      form.expiresAt !== emptyFormState.expiresAt ||
      form.scopes.length !== emptyFormState.scopes.length ||
      form.scopes.some((scope, index) => scope !== emptyFormState.scopes[index])
    );
  }, [form]);
  ...
  const allowedScopes = listQuery.data?.allowedScopes?.length
    ? listQuery.data.allowedScopes
    : DEFAULT_SCOPE_OPTIONS.map((option) => option.value);
  ...
  async function handleRevoke(id: string) {
    ...
    try {
      await revokeMutation.mutateAsync({ id, tenantId });
      toast({
  ...
  if (!tenantId) {
    return (
      <PageState
        ...
        description="Selecione um tenant acima para gerenciar tokens."
      />
    );
  }
    ...
    <div className="grid min-w-0 gap-6">
      <Card variant="glass" padding="lg" className="min-w-0">
        ...
        <CardDescription className="mt-2">
          Crie e revogue tokens deste tenant.
        </CardDescription>
        <form className="mt-5 grid min-w-0 gap-4" onSubmit={handleCreate}>
          ...
          <div className="grid gap-3">
            <p className="text-xs text-text-soft">
              Use o header <span className="font-semibold text-white">Authorization: Bearer {"<TOKEN>"}</span> no HTTP Request do n8n.
            </p>
      ...
      {oneTimeToken ? (
        <Card variant="premium" padding="lg" className="min-w-0">
          ...
          <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <Button type="button" size="md" className="w-full md:w-auto" onClick={() => void copyOneTimeToken()}>
              <Copy size={16} />
              Copiar token
            </Button>
            <Button
